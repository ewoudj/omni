...
exports.listen = function(port, collections, events, options) {
    ...
    webSocketServer.sockets.on("connection", function (socket) {
    	...
    	(function createConnection(callback){
			...
			result.resync = result.sync = result.recheckAllPermissions;
			if (events != undefined && events.connect != null) {
				events.connect.run(connection, collections, null, function(eventOutput){
					callback(result);
				});
        	}
        	else {
			...
			for (var k in events) {
				...
				(function(event, k) {
					socket.on("event:" + k, function (data) {
						event.run(connection, collections, data, function(eventResponse){
			...
			socket.on("disconnect", function() {
				if (events != undefined && events.disconnect != null) {
					events.disconnect.run(connection, collections, null, function(){
